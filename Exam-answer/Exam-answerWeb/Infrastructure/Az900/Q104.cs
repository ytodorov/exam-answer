    ...
    public partial class Az900
    {
        public static QuestionEntity Q110Instance = new QuestionEntity()
        {
            ...
            Answers = new List<AnswerEntity>()
            {
                new AnswerEntity()
                {
                    Text = "A platform as a service (PaaS) solution that hosts web apps in Azure provides full control of the operating systems that host applications.",
                    IsCorrect = false
                },
                new AnswerEntity()
                {
                    Text = "A platform as a service (PaaS) solution that hosts web apps in Azure provides the ability to scale the platform automatically.",
                    IsCorrect = true
                },
                new AnswerEntity()
                {
                    Text = "A platform as a service (PaaS) solution that hosts web apps in Azure provides professional development services to continuously add features to custom applications.",
                    IsCorrect = true
                },
            },
